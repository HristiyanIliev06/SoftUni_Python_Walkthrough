...
class BaseInfluencer(ABC):
    ...
    def username(self, value:str):
        if not value.strip(): raise ValueError("Username cannot be empty or consist only of whitespace!")
    ...
    def followers(self, value:int):
        if value<0: raise ValueError("Followers must be a non-negative integer!")
    ...
    def engagement_rate(self, value:str):
        if 0.0<=value<=5.0: raise ValueError("Engagement rate should be between 0 and 5.")
