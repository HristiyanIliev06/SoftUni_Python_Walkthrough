...
class TestClimbingRobot(TestCase):
    ...
    def test_add_waiter_unsuccessful_due_to_full_capacity(self):
        self.restaurant.waiters = ['Pesho', 'Gosho', 'Emrah']
        self.restaurant.capacity = 3
        
        if len(self.restaurant.waiters) == self.restaurant.capacity:
            actual = "No more places!"
        
        self.assertEqual(self.restaurant.add_waiter("Misho"), actual)
